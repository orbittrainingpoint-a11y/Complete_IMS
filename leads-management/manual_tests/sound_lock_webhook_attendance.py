...
try:
    ...
    for who in ('zz_sales', 'zz_admin'):
        ...
        r = c.post('/sound-check', data={'audio_ok': '0', 'next': '/'})
        ...
        c.post('/sound-check', data={'audio_ok': '1', 'next': '/'})
        c.get('/logout')
        c2, _ = login(who)
        check(f'{who}: a new login starts locked again', c2.get('/leads', headers=HTML).status_code == 302)
    c, _ = login('zz_sales'); c.post('/sound-check', data={'audio_ok': '1'})
    r = c.post('/sound-check', data={'audio_ok': '1', 'next': 'https://evil.example'})
    ...
    c, _ = login('zz_admin'); c.post('/sound-check', data={'audio_ok': '1'})
